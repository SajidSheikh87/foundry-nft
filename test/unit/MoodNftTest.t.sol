// SPDX-License-Identifier: MIT
pragma solidity ^0.8.18;

import {Test, console} from "forge-std/Test.sol";
import {MoodNft} from "../../src/MoodNft.sol";

contract MoodNftTest is Test {
    MoodNft moodNft;
    string public constant HAPPY_SVG_URL =
        "data:image/svg+xml;base64,";
    string public constant SAD_SVG_URL =
        "data:image/svg+xml;base64,";

    address USER = makeAddr("user");

    function setUp() public {
        moodNft = new MoodNft(SAD_SVG_URL, HAPPY_SVG_URL);
    }

    modifier mintAtoken() {
        vm.prank(USER);
        moodNft.mintNft();
        _;
    }

    function testViewTokenURI() public mintAtoken {
        console.log(moodNft.tokenURI(0));
    }

    function testGetTotalTokensCount() public mintAtoken {
        assert(moodNft.getTotalTokensCount() == 0);
    }

    function testGetMood() public mintAtoken {
        string memory defaultMood = "HAPPY";
        assertEq(moodNft.getMood(0), defaultMood);
    }
}
